...
public class ZooKeeper {
    ...
    protected final ClientCnxn cnxn; //核心处理类
    ...
    private final ZKWatchManager watchManager = new ZKWatchManager(); //创建监听管理
     ...
     * API.
     */
    private static class ZKWatchManager implements ClientWatchManager { //客户端watch管理实现
        ...
        public Set<Watcher> materialize(Watcher.Event.KeeperState state,
                                        ...
                                        String clientPath)
        {
            Set<Watcher> result = new HashSet<Watcher>(); //所有的观察者的集合
            ...
            case None: //无状态
                ...
                synchronized(dataWatches) { //如果是数据改变
                ...
                synchronized(existWatches) {  //如果是节点改变
                ...
                synchronized(childWatches) { //如果是孩子节点改变
            ...
            case NodeCreated: //节点创建
                ...
                break;
            case NodeChildrenChanged: //孩子节点改变
                synchronized (childWatches) {
                    addTo(childWatches.remove(clientPath), result);
                }
                break;
            case NodeDeleted: //节点删除
                synchronized (dataWatches) {
                    addTo(dataWatches.remove(clientPath), result);
                }
     ...
     */
    abstract class WatchRegistration { //客户端注册watch
        private Watcher watcher; //注册时watch
        private String clientPath; //监听的path
        public WatchRegistration(Watcher watcher, String clientPath)
        {
            this.watcher = watcher;
            this.clientPath = clientPath;
        }
        //获取对应的Watcher的Map 客户端观察管理者中的
        ...
        public void register(int rc) { //
            if (shouldAddWatch(rc)) { //如果可以添加
                Map<String, Set<Watcher>> watches = getWatches(rc); //获取所有注册的path和watcher映射
                synchronized(watches) {//锁定
                    Set<Watcher> watchers = watches.get(clientPath); //获取path上的监听
                    if (watchers == null) { //如果watcher等于空 创建path对应的watcher集合
    ...
    class DataWatchRegistration extends WatchRegistration {
        ...
        @Override
        protected Map<String, Set<Watcher>> getWatches(int rc) {
            return watchManager.dataWatches;//返回全部的datawatcher
        }
    }
        ...
        throws IOException
    {
        ...
        watchManager.defaultWatcher = watcher; //设置默认的watcher
    ...
    public byte[] getData(final String path, Watcher watcher, Stat stat)
     ...
     {
        final String clientPath = path;
        PathUtils.validatePath(clientPath); //校验path合理性

        // the watch contains the un-chroot path
        WatchRegistration wcb = null;
        if (watcher != null) { //如果watcher不为空则注册
        ...
        GetDataRequest request = new GetDataRequest();//生成获取数据请求
        request.setPath(serverPath); //设置path
        request.setWatch(watcher != null);//如果有watcher设置watcher标志
        GetDataResponse response = new GetDataResponse();//获取响应
        ReplyHeader r = cnxn.submitRequest(h, request, response, wcb); //client提交请求
        if (r.getErr() != 0) {
            throw KeeperException.create(KeeperException.Code.get(r.getErr()),
                    clientPath);
        }
    ...
    private static ClientCnxnSocket getClientCnxnSocket() throws IOException {
        ...
        if (clientCnxnSocketName == null) {
            clientCnxnSocketName = ClientCnxnSocketNIO.class.getName(); //默认使用ClientCnxnSocketNIO作为客户端实现
        }
        try {
