...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    protected int minSessionTimeout = -1; //最小的session超时时间
    /** value of -1 indicates unset, use default */
    protected int maxSessionTimeout = -1; //最大的session超时时间
    protected SessionTracker sessionTracker; //session跟踪器
    private FileTxnSnapLog txnLogFactory = null;//事务快照，日志
    private ZKDatabase zkDb;//zk数据库
    private final AtomicLong hzxid = new AtomicLong(0); //zxid
    public final static Exception ok = new Exception("No prob");//正常异常
    protected RequestProcessor firstProcessor;//第一个请求处理器
    ...
    final HashMap<String, ChangeRecord> outstandingChangesForPath =
        new HashMap<String, ChangeRecord>();//变更path和变更记录的映射
    
    private ServerCnxnFactory serverCnxnFactory;//连接工厂

    private final ServerStats serverStats; //服务器状态统计

    private final ZooKeeperServerListener listener = new ZooKeeperServerListenerImpl(); //监听，处理服务器stop状态
    ...
    public void loadData() throws IOException, InterruptedException {
        ...
        for (Long session : zkDb.getSessions()) {//获取会话记录
            if (zkDb.getSessionWithTimeOuts().get(session) == null) { //session是否已经超时
        ...
        for (long session : deadSessions) {
            ...
            killSession(session, zkDb.getDataTreeLastProcessedZxid()); //killsession
        }
    }
    ...
    throws IOException, InterruptedException {
        ...
        if (zkDb == null) { //检查数据库不存在创建数据库
            zkDb = new ZKDatabase(this.txnLogFactory); 
        }  
        if (!zkDb.isInitialized()) {//内存数据库未初始化过，进行初始化
            loadData();
        }
    }
    ...
    protected void setupRequestProcessors() {
        ...
        ((PrepRequestProcessor)firstProcessor).start(); //启动处理程序
    }
    ...
    public synchronized void shutdown() {
        ...
        state = State.SHUTDOWN; //设置停止状态
        // Since sessionTracker and syncThreads poll we just have to
        // set running to false and they will detect it during the poll
        // interval.
        if (sessionTracker != null) {
            sessionTracker.shutdown(); //session追踪器停止
        }
        if (firstProcessor != null) {
            firstProcessor.shutdown(); //处理请求实现类停止
        }
        if (zkDb != null) {
            zkDb.clear(); //清除ZK数据库
        }
    ...
    static class ChangeRecord {
        ...
        ChangeRecord duplicate(long zxid) { //复制
    ...
    protected void revalidateSession(ServerCnxn cnxn, long sessionId,
        ...
        boolean rc = sessionTracker.touchSession(sessionId, sessionTimeout); //校验session是否有效
        if (LOG.isTraceEnabled()) {
            ZooTrace.logTraceMessage(LOG,ZooTrace.SESSION_TRACE_MASK,
                                     "Session 0x" + Long.toHexString(sessionId) +
                    " is valid: " + rc);
        }
        finishSessionInit(cnxn, rc); //有效进行重连
    }

    public void reopenSession(ServerCnxn cnxn, long sessionId, byte[] passwd,
            int sessionTimeout) throws IOException {
        if (!checkPasswd(sessionId, passwd)) { //检查用户密码
            finishSessionInit(cnxn, false);
        } else {
            revalidateSession(cnxn, sessionId, sessionTimeout);//校验session
        }
    }

    public void finishSessionInit(ServerCnxn cnxn, boolean valid) { //重连
        // register with JMX
        try {
            if (valid) {
        ...
        try {
            ...
            if (!valid) {
                LOG.info("Invalid session 0x"
                        ...
                        + ", probably expired");
                cnxn.sendBuffer(ServerCnxnFactory.closeConn); //关闭连接
            } else {
    ...
    public void processConnectRequest(ServerCnxn cnxn, ByteBuffer incomingBuffer) throws IOException {
        ...
        connReq.deserialize(bia, "connect"); //反序列化连接请求
        if (LOG.isDebugEnabled()) {
        ...
        try {
            readOnly = bia.readBool("readOnly"); //读取否只读模式
            cnxn.isOldClient = false; //设置不为旧的客户端
        } catch (IOException e) {
        ...
        if (readOnly == false && this instanceof ReadOnlyZooKeeperServer) { //如果模式为只读模式并且客户端为ReadOnlyZooKeeperServer 直接不处理拒绝
        ...
        int sessionTimeout = connReq.getTimeOut(); //获取session超时时间
        byte passwd[] = connReq.getPasswd();
        int minSessionTimeout = getMinSessionTimeout(); //获取最小的session超时时间
        ...
        cnxn.setSessionTimeout(sessionTimeout); //设置超时时间
        ...
        if (sessionId != 0) { //连接请求的sessionId不为空说明之前已经连接过了 这里代表重新连接
            ...
            LOG.info("Client attempting to establish new session at "
                    + cnxn.getRemoteSocketAddress());
            createSession(cnxn, passwd, sessionTimeout); //创建sessionId
        }
    }
    ...
    public void processPacket(ServerCnxn cnxn, ByteBuffer incomingBuffer) throws IOException {
         ...
         * 请求头：具体数据结构为
         * -----------------------------------
         * |0-3|4-7|8-11|12-15|16-(n-1)|  n  |
         * -----------------------------------
         * |len|xid|type| len |  path  |watch|
         * -----------------------------------
         * 第一个len为整个数据长度在调用该方法之前已经取出设置为buffer大小
         * xid为序号
         * type操作类型
         * 第二个len为数据长度
         * path路径
        ...
        if (h.getType() == OpCode.auth) { //添加权限信息
            LOG.info("got auth packet " + cnxn.getRemoteSocketAddress());
            AuthPacket authPacket = new AuthPacket();
            ByteBufferInputStream.byteBuffer2Record(incomingBuffer, authPacket);
            String scheme = authPacket.getScheme();
            AuthenticationProvider ap = ProviderRegistry.getProvider(scheme); //获取校验模式实现类
