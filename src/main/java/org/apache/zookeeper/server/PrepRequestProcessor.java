...
public class PrepRequestProcessor extends ZooKeeperCriticalThread implements
    ...
    LinkedBlockingQueue<Request> submittedRequests = new LinkedBlockingQueue<Request>(); //处理请求的堵塞队列
    ...
    public void run() {
        try {
            while (true) {
                Request request = submittedRequests.take(); //链表阻塞队列
    ...
    static void checkACL(ZooKeeperServer zks, List<ACL> acl, int perm,
        ...
        for (Id authId : ids) {
            if (authId.getScheme().equals("super")) { //如果是super权限直接返回
                return;
            }
        }
        for (ACL a : acl) {
            Id id = a.getId();
            if ((a.getPerms() & perm) != 0) { //如果对应的节点有perm权限进行验证
                if (id.getScheme().equals("world")
                        && id.getId().equals("anyone")) { //如果是world也直接返回
                    return;
                }
                AuthenticationProvider ap = ProviderRegistry.getProvider(id
                        .getScheme());
                if (ap != null) { //如果有一个有权限则通过
                    for (Id authId : ids) {                        
                        if (authId.getScheme().equals(id.getScheme()) //匹配模式
                                && ap.matches(authId.getId(), id.getId())) {//匹配权限
                            return;
                        }
                    }
                }
        ...
        throw new KeeperException.NoAuthException();//否则抛出异常
    }
        ...
        throws KeeperException, IOException, RequestProcessorException
    {
        ...
        switch (type) {
            case OpCode.create:                
                ...
                CreateRequest createRequest = (CreateRequest)record;  //将请求装换为CreateRequest
                if(deserialize) //是否需要反序列化 因为通过网络传输需要进行反序列化
                    ByteBufferInputStream.byteBuffer2Record(request.request, createRequest);
                String path = createRequest.getPath(); //获取创建节点的path
                int lastSlash = path.lastIndexOf('/');
                ...
                List<ACL> listACL = removeDuplicates(createRequest.getAcl()); //移除重复的ACL信息
                if (!fixupACL(request.authInfo, listACL)) { //ACL创建和修改
                    throw new KeeperException.InvalidACLException(path);
                }
                String parentPath = path.substring(0, lastSlash); //父节点路径
                ChangeRecord parentRecord = getRecordForPath(parentPath); //获取父节点

                checkACL(zks, parentRecord.acl, ZooDefs.Perms.CREATE,
                        request.authInfo); //检查是否有创建权限
                int parentCVersion = parentRecord.stat.getCversion(); //父节点获取属性子节点变动版本
                CreateMode createMode =
                    CreateMode.fromFlag(createRequest.getFlags()); //获取创建模式
                if (createMode.isSequential()) { //如果节点类型是序列化则使用父节点CVersion的版本号进行序列化
                    path = path + String.format(Locale.ENGLISH, "%010d", parentCVersion); //
                }
                validatePath(path, request.sessionId); //校验路径
                try {
                ...
                if (ephemeralParent) {//如果是临时节点没有所属的Session抛出异常（临时节点信息是和session绑定）
                    throw new KeeperException.NoChildrenForEphemeralsException(path);
                }
                int newCversion = parentRecord.stat.getCversion()+1; //父节点孩子版本号变化
                request.txn = new CreateTxn(path, createRequest.getData(),
                        listACL,
                        createMode.isEphemeral(), newCversion); //创建节点事务
                StatPersisted s = new StatPersisted(); //统计信息
                if (createMode.isEphemeral()) { //如果是临时节点
                    s.setEphemeralOwner(request.sessionId); //设置统计信息中的所属session
                }
                parentRecord = parentRecord.duplicate(request.hdr.getZxid());
                parentRecord.childCount++;
                parentRecord.stat.setCversion(newCversion);
                addChangeRecord(parentRecord); //修改父节点
            ...
            case OpCode.delete:
                ...
                if(deserialize)
                    ByteBufferInputStream.byteBuffer2Record(request.request, deleteRequest); //序列化转成对象
                path = deleteRequest.getPath(); //获取路径
                lastSlash = path.lastIndexOf('/');
                if (lastSlash == -1 || path.indexOf('\0') != -1
                        || zks.getZKDatabase().isSpecialPath(path)) {
                    throw new KeeperException.BadArgumentsException(path);
                }
                parentPath = path.substring(0, lastSlash); //获取父路径
            ...
            case OpCode.setData:
                ...
                if(deserialize)
                    ByteBufferInputStream.byteBuffer2Record(request.request, setDataRequest);//序列化转成对象
                path = setDataRequest.getPath(); //获取路径
                validatePath(path, request.sessionId); //校验路径
    ...
    protected void pRequest(Request request) throws RequestProcessorException {
        ...
        try {
            switch (request.type) {
                ...
                pRequest2Txn(request.type, zks.getNextZxid(), request, createRequest, true); //如果是事务请求将事务添加进request中
                break;
            case OpCode.delete:
            ...
            //create/close session don't require request record 非事务请求验证session
            case OpCode.createSession:
            case OpCode.closeSession:
    ...
    private boolean fixupACL(List<Id> authInfo, List<ACL> acl) {
        ...
        //如果ACL信息不存在返回false 创建节点的时候是否有默认的ACL信息？ 默认使用信息为new ACL(Perms.ALL, ANYONE_ID_UNSAFE) 任何用户均可访问
        if (acl == null || acl.size() == 0) {
            return false;
        }

        Iterator<ACL> it = acl.iterator();//获取权限信息迭代
        LinkedList<ACL> toAdd = null;
        while (it.hasNext()) {
            ACL a = it.next();
            Id id = a.getId();
            if (id.getScheme().equals("world") && id.getId().equals("anyone")) { //默认权限
                // wide open
            } else if (id.getScheme().equals("auth")) { //模式为auth 不使用任何id，代表任何已认证的用户 连接即认证
    ...
    public void processRequest(Request request) { //添加请求至队列
