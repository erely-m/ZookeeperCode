...
public class NIOServerCnxn extends ServerCnxn { //会话上下文类
    ...
    ByteBuffer lenBuffer = ByteBuffer.allocate(4); //长度头

    ByteBuffer incomingBuffer = lenBuffer;

    LinkedBlockingQueue<ByteBuffer> outgoingBuffers = new LinkedBlockingQueue<ByteBuffer>(); //发送队列

    int sessionTimeout; //session超时

    private final ZooKeeperServer zkServer;//zkServer服务器实现类
    ...
    static long nextSessionId = 1; //下一个会话
    ...
    public void sendBuffer(ByteBuffer bb) {
        try {
            if (bb != ServerCnxnFactory.closeConn) { //不为关闭
            ...
            synchronized(this.factory){
                ...
                outgoingBuffers.add(bb); //添加到发送缓存队列 使用NIO方式进行发送
                if (sk.isValid()) {
                    sk.interestOps(sk.interestOps() | SelectionKey.OP_WRITE); //注册可写
                }
            }
    ...
    private void readPayload() throws IOException, InterruptedException {
        if (incomingBuffer.remaining() != 0) { // have we read length bytes?  数据可读
            int rc = sock.read(incomingBuffer); // sock is non-blocking, so ok 读取数据
            if (rc < 0) {
        ...
        if (incomingBuffer.remaining() == 0) { // have we read length bytes?   收到了长度头长度的数据
            packetReceived();
            incomingBuffer.flip(); //切换读模式
            if (!initialized) { //如果为初始化进行处理连接请求
                readConnectRequest();//连接请求
            } else {
                readRequest();//请求数据
            }
            lenBuffer.clear(); //清除buffer
            incomingBuffer = lenBuffer; //设置为读取长度头buffer
        }
    }
    ...
    /**
     * Handles read/write IO on connection.数据交互
     */
    void doIO(SelectionKey k) throws InterruptedException {
        try {
            ...
            if (k.isReadable()) {
                int rc = sock.read(incomingBuffer); //读长度
                if (rc < 0) {
                    throw new EndOfStreamException(
                            "Unable to read additional data from client sessionid 0x"
                            + Long.toHexString(sessionId)
                ...
                if (incomingBuffer.remaining() == 0) { //正好读完
                    boolean isPayload;
                    if (incomingBuffer == lenBuffer) { // start of next request 如果是长度头
                        incomingBuffer.flip();
                        isPayload = readLength(k);//创建长度头长度的缓冲区
                        incomingBuffer.clear(); //清除incomingBuffer
                    } else {
                        // continuation
                        isPayload = true;
                    }
                    if (isPayload) { // not the case for 4letterword 读完长度头之后值为true
                        readPayload(); //读取数据
                    }
                    else {
            ...
            if (k.isWritable()) { //如果可以写入
                ...
                if (outgoingBuffers.size() > 0) {//发送队列不为空
                    ...
                    for (ByteBuffer b : outgoingBuffers) { //依次处理队列中的待发送数据
                        if (directBuffer.remaining() < b.remaining()) { //分片处理？
                            /*
                        ...
                        directBuffer.put(b);
                        b.position(p);
                        if (directBuffer.remaining() == 0) { //如果写满了
                            break;
                        }
                    }
                    /*
                    ...
                    while (outgoingBuffers.size() > 0) {
                        ...
                        sent -= bb.remaining();
                        outgoingBuffers.remove(); //从队列移除该条记录
                    }
                    // ZooLog.logTraceMessage(LOG,
                ...
                synchronized(this.factory){
                    if (outgoingBuffers.size() == 0) { //如果队列为空
                        if (!initialized
                                && (sk.interestOps() & SelectionKey.OP_READ) == 0) {
                            throw new CloseRequestException("responded to info probe");
                        }
                        sk.interestOps(sk.interestOps()
                                & (~SelectionKey.OP_WRITE)); //设置可写
                    } else {
                        sk.interestOps(sk.interestOps()
                                | SelectionKey.OP_WRITE); //设置可写
                    }
    ...
    private boolean readLength(SelectionKey k) throws IOException {
        ...
        if (!initialized && checkFourLetterWord(sk, len)) { //检查读取的数据 是否是有效荷载
            return false;
        }
        ...
        incomingBuffer = ByteBuffer.allocate(len); //开辟空间 进行数据读取
        return true;
    }
     ...
     */
    @Override
    synchronized public void process(WatchedEvent event) { //触发事件，通知客户端
        ...
        sendResponse(h, e, "notification"); //发送通知给客户端
    }
    ...
    @Override
    protected ServerStats serverStats() { //服务器状态统计
        if (zkServer == null) {
            return null;
        }
