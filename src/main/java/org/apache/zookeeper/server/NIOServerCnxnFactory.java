...
public class NIOServerCnxnFactory extends ServerCnxnFactory implements Runnable {
    ...
    */
    ...
    public void configure(InetSocketAddress addr, int maxcc) throws IOException {
        configureSaslLogin(); //权限控制

        thread = new ZooKeeperThread(this, "NIOServerCxn.Factory:" + addr); //创建zookeeper线程（服务端线程）
        thread.setDaemon(true); //设置为守护线程
        maxClientCnxns = maxcc;
        this.ss = ServerSocketChannel.open(); //打开选择器
        ss.socket().setReuseAddress(true);
        LOG.info("binding to port " + addr);
        ss.socket().bind(addr); //绑定地址以及IP
        ss.configureBlocking(false);
        ss.register(selector, SelectionKey.OP_ACCEPT); //注册NIO监听事件
    }
    ...
    public void run() {
        while (!ss.socket().isClosed()) {
            try {
                selector.select(1000); //最坏结果等待1s
                Set<SelectionKey> selected;
                synchronized (this) {
                    selected = selector.selectedKeys(); //获取所有事件
                }
