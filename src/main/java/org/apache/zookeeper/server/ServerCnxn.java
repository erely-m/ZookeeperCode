...
public abstract class ServerCnxn implements Stats, Watcher {
    ...
    final public static Object me = new Object(); // 代表由本类提出的请求
     ...
     */
    boolean isOldClient = true; //是否为旧的客户端
    ...
    public void addAuthInfo(Id id) { //权限认证信息 id
    ...
    protected ZooKeeperSaslServer zooKeeperSaslServer = null; //zookeeper的sasl服务器
    ...
    protected void packetReceived() {
        ...
        if (serverStats != null) {
            serverStats().incrementPacketsReceived(); //增加接收次数
        }
    }

    protected void packetSent() {
        incrPacketsSent(); //统计信息添加
        ServerStats serverStats = serverStats();
        if (serverStats != null) {
            serverStats().incrementPacketsSent();//统计信息添加
        }
    }

    protected abstract ServerStats serverStats();
    
    protected final Date established = new Date(); //创建连接的时间

    protected final AtomicLong packetsReceived = new AtomicLong(); //接收次数
    protected final AtomicLong packetsSent = new AtomicLong(); //发送次数

    protected long minLatency; //最小延迟
    protected long maxLatency; //最大延迟
    protected String lastOp;//最后的操作类型
    protected long lastCxid; //最后的Cxid
    protected long lastZxid;//最后的Zxid
    protected long lastResponseTime; //最后响应时间
    protected long lastLatency; //最后的延迟

    protected long count; //数量？？？？
    protected long totalLatency;//延迟总数
    ...
    protected long incrPacketsReceived() {
        return packetsReceived.incrementAndGet(); //增加接收次数
    }
