...
public class PurgeTxnLog {
     ...
     * @throws IOException
     */
    public static void purge(File dataDir, File snapDir, int num) throws IOException { //清除快照日志、交易日志
        if (num < 3) { //保存的快照数量不能小于3
            throw new IllegalArgumentException(COUNT_ERR_MSG);
        }

        FileTxnSnapLog txnLog = new FileTxnSnapLog(dataDir, snapDir);

        List<File> snaps = txnLog.findNRecentSnapshots(num); //对不需要删除的文件进行保存
        retainNRecentSnapshots(txnLog, snaps);
    }

    // VisibleForTesting
    static void retainNRecentSnapshots(FileTxnSnapLog txnLog, List<File> snaps) { //保留最近的N个快照
        // found any valid recent snapshots?
        if (snaps.size() == 0)
            return;
        File snapShot = snaps.get(snaps.size() -1); //获取最后一个快照文件
        final long leastZxidToBeRetain = Util.getZxidFromName(
                snapShot.getName(), PREFIX_SNAPSHOT); //获取字后需要保存的Zxid

        class MyFileFilter implements FileFilter{ //不需要保存的文件拦截器
            ...
            public boolean accept(File f){
                ...
                if (fZxid >= leastZxidToBeRetain) { //如果文件的zxId大于需要最后一个文件的zxid则不进行处理否则需要删除
                    return false;
                }
                return true;
            }
