...
public class DigestAuthenticationProvider implements AuthenticationProvider {
        ...
        handleAuthentication(ServerCnxn cnxn, byte[] authData)
    {
        ...
        try {
            String digest = generateDigest(id); //获取用户密码 user:password
            if (digest.equals(superDigest)) { //如果为管理员用户密码则设置权限为管理员
                cnxn.addAuthInfo(new Id("super", ""));
            }
            cnxn.addAuthInfo(new Id(getScheme(), digest)); //添加ID信息
    ...
    public boolean isValid(String id) { //校验是否合理
