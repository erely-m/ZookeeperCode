...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    private InetSocketAddress myQuorumAddr; //节点地址

    public InetSocketAddress getQuorumAddress(){
        return myQuorumAddr;
    }

    private int electionType; //选举类型
    ...
    private void loadDataBase() {
        File updating = new File(getTxnFactory().getSnapDir(),
                                 UPDATING_EPOCH_FILENAME); //获取快照目录
		try {
            zkDb.loadDataBase();
    ...
    synchronized public void startLeaderElection() {
    	try {
    		currentVote = new Vote(myid, getLastLoggedZxid(), getCurrentEpoch()); //myId ZXid 时代
    	} catch(IOException e) {
    		RuntimeException re = new RuntimeException(e.getMessage());
    		re.setStackTrace(e.getStackTrace());
    		throw re;
    	}
        for (QuorumServer p : getView().values()) { //获取所有的集群服务器
            if (p.id == myid) { //如果集群id等于自己的Myid
                myQuorumAddr = p.addr; //设置节点地址
                break;
            }
        }
            ...
            while (running) {
                ...
                case LOOKING: //没有主 进行选举
                    ...
                    if (Boolean.getBoolean("readonlymode.enabled")) {
                        ...
                        try {
                            setBCVote(null);
                            setCurrentVote(makeLEStrategy().lookForLeader()); //开始进行选举
                        } catch (Exception e) {
                            LOG.warn("Unexpected exception", e);
                            setPeerState(ServerState.LOOKING);
                        }
                ...
                case LEADING:
                    LOG.info("LEADING");
                    try {
                        setLeader(makeLeader(logFactory)); //设置为主
                        leader.lead(); //启动通讯
                        setLeader(null);
                    } catch (Exception e) {
                        LOG.warn("Unexpected exception",e);
