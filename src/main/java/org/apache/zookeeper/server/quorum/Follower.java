...
public class Follower extends Learner{
    ...
    void followLeader() throws InterruptedException {
        ...
        try {
            InetSocketAddress addr = findLeader();   //寻找leader
            try {
                connectToLeader(addr); //连接leader
                long newEpochZxid = registerWithLeader(Leader.FOLLOWERINFO); //注册leader
                ...
                syncWithLeader(newEpochZxid);    //同步leader
                QuorumPacket qp = new QuorumPacket();
                while (self.isRunning()) { //接收leader的数据
                    readPacket(qp);
                    processPacket(qp); //处理
                }
            } catch (Exception e) {
