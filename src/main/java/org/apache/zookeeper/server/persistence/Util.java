...
public class Util {
    ...
    public static boolean isValidSnapshot(File f) throws IOException {
        if (f==null || Util.getZxidFromName(f.getName(), "snapshot") == -1) //检查名字
            return false;

        // Check for a valid snapshot
        RandomAccessFile raf = new RandomAccessFile(f, "r"); //读取文件
        try {
    ...
    public static long padLogFile(FileOutputStream f,long currentSize,
        ...
        long position = f.getChannel().position(); //获取文件当前位置
        if (position + 4096 >= currentSize) { //如果文件大小剩余不足4K
            currentSize = currentSize + preAllocSize;//开辟空间以0填充每次添加的大小是preAllocSize（默认值）64M
            fill.position(0); //用0填充空间
    ...
    public static byte[] readTxnBytes(InputArchive ia) throws IOException {
        try{
            byte[] bytes = ia.readBuffer("txtEntry"); //通过长度头读取事务
