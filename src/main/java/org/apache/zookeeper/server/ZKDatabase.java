...
public class ZKDatabase {
     ...
     * all these members.
     */
    protected DataTree dataTree; //dataTree数据
    protected ConcurrentHashMap<Long, Integer> sessionsWithTimeouts; //session超时时间
    protected FileTxnSnapLog snapLog;	//事务、快照日志
    protected long minCommittedLog, maxCommittedLog; //最小的提交记录，最大的提交记录 都是zxid 用于使用那种方式进行数据同步
    public static final int commitLogCount = 500; //内存事务日志总量
    protected static int commitLogBuffer = 700;
    protected LinkedList<Proposal> committedLog = new LinkedList<Proposal>(); //提交记录
    protected ReentrantReadWriteLock logLock = new ReentrantReadWriteLock();
    volatile private boolean initialized = false; //是否初始化标志
    ...
    public long loadDataBase() throws IOException {
        PlayBackListener listener=new PlayBackListener(){
            public void onTxnLoaded(TxnHeader hdr,Record txn){
                ...
                r.hdr = hdr;
                r.zxid = hdr.getZxid();
                addCommittedProposal(r); //添加提交记录 TODO
            }
        };
        
        long zxid = snapLog.restore(dataTree,sessionsWithTimeouts,listener);
        initialized = true; //设置恢复标志
        return zxid;
    }
    ...
    public void addCommittedProposal(Request request) {
        ...
        try {
            wl.lock();
            if (committedLog.size() > commitLogCount) { //如果提交记录的大小大于提交记录的阈值
                committedLog.removeFirst();//移除一个元素
                minCommittedLog = committedLog.getFirst().packet.getZxid(); //设置当前最小的提交记录的zxid为新的
            }
            if (committedLog.size() == 0) { //如果为第一次则将最大最小的赋值为当前zxid
